...
TABS.gps.initialize = function (callback) {
  ...
  function load_html() {
    GUI.load(
      ...
      Settings.processHtml(process_html)
    );
  }
  ...
  function process_html() {
    ...
    mapHandler.on('pointermove', function (evt) {
      var feature = mapHandler.forEachFeatureAtPixel(
        mapHandler.getEventPixel(evt.originalEvent),
      ...
      if (feature && feature.get('data') && feature.get('name')) {
        TABS.gps.toolboxAdsbVehicle
          .setContent(
            `callsign: <strong>` +
              feature.get('name') +
              `</strong><br />` +
              `lat: <strong>` +
              feature.get('data').lat / 10000000 +
              `</strong><br />` +
              `lon: <strong>` +
              feature.get('data').lon / 10000000 +
              `</strong><br />` +
              `ASL: <strong>` +
              feature.get('data').altCM / 100 +
              `m</strong><br />` +
              `heading: <strong>` +
              feature.get('data').headingDegrees +
              `°</strong><br />` +
              `type: <strong>` +
              ADSB_VEHICLE_TYPE[feature.get('data').emitterType].name +
              `</strong>`
          )
          .open();
      } else {
    ...
    function get_raw_adsb_data() {
      MSP.send_message(
        MSPCodes.MSP2_ADSB_VEHICLE_LIST,
        false,
        false,
        update_adsb_ui
      );
    ...
    function update_adsb_ui() {
      ...
      $('.adsbHeartbeatTotalMessages').html(
        FC.ADSB_VEHICLES.heartbeatPacketCount
      );
      ...
      for (let key in FC.ADSB_VEHICLES.vehicles) {
        ...
        if (vehicle.lat != 0 && vehicle.lon != 0 && vehicle.ttl > 0) {
          let vehicleIconStyle = new ol.style.Style({
            image: new ol.style.Icon({
              ...
              src: path.join(
                __dirname,
                './../resources/adsb/' +
                  ADSB_VEHICLE_TYPE[vehicle.emitterType].icon
              )
            }),
          ...
          let iconGeometry = new ol.geom.Point(
            ol.proj.fromLonLat([vehicle.lon / 10000000, vehicle.lat / 10000000])
          );
    ...
    if (semver.gte(FC.CONFIG.flightControllerVersion, '8.0.0')) {
      ...
      mspHelper.loadSerialPorts(function () {
        for (var i in FC.SERIAL_CONFIG.ports) {
          if (
            FC.SERIAL_CONFIG.ports[i].functions &&
            FC.SERIAL_CONFIG.ports[i].functions.includes('TELEMETRY_MAVLINK')
          ) {
            ...
            if (totalSent % 100 == 0) {
              GUI.log(
                totalSent +
                  ...
                  total +
                  ' ' +
                  i18n.getMessage('gpsAssistnowUpdate')
              );
            }
            if (callCallback) {
              callback();
    ...
    $('a.loadAssistnowOnline').on('click', function () {
      if (
        globalSettings.assistnowApiKey != null &&
        globalSettings.assistnowApiKey != ''
      ) {
