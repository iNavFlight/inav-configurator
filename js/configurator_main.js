...
function buildGoogleApiHelpHtml() {
    return '<div id="googleApiHelpBackdrop" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:10000;display:flex;align-items:center;justify-content:center;">'
...
function showGoogleApiHelpDialog(e) {
    e.preventDefault();

    $('body').append(buildGoogleApiHelpHtml());
    $('#googleApiHelpBackdrop').on('click', function (event) {
        if (event.target === this) {
            $(this).remove();
        }
    });
    $('#googleApiHelpClose').on('click', function (event) {
        event.preventDefault();
        $('#googleApiHelpBackdrop').remove();
    });
}

function showGoogleApiTestResult($result, text, css, useHtml = false) {
    const setter = useHtml ? 'html' : 'text';
    $result[setter](text).css(css).show();
}

function appendGoogleWeatherResult(apiKey, locationData, locationMessage, $result) {
    const weatherUrl = 'https://weather.googleapis.com/v1/currentConditions:lookup?key=' + apiKey
        + '&location.latitude=' + locationData.location.lat
        + '&location.longitude=' + locationData.location.lng;

    return fetch(weatherUrl)
        .then(function (weatherResponse) { return weatherResponse.json(); })
        .then(function (weatherData) {
            if (weatherData.wind?.speed) {
                const windValue = weatherData.wind.speed.value;
                const windUnit = weatherData.wind.speed.unit === 'KILOMETERS_PER_HOUR' ? 'km/h' : 'mph';
                const weatherMessage = i18n.getMessage('googleLocationTestWeather', [windValue, windUnit]);
                showGoogleApiTestResult($result, locationMessage + '<br>' + weatherMessage, { color: '#080', background: '#f0fff0' }, true);
                return;
            }

            const weatherError = weatherData.error?.message || 'Unexpected response';
            const warningMessage = i18n.getMessage('googleLocationTestWeatherFail', [weatherError]);
            showGoogleApiTestResult($result, locationMessage + '<br><span style="color:#c00;">' + warningMessage + '</span>', { color: '#080', background: '#f0fff0' }, true);
        })
        .catch(function (weatherError) {
            const warningMessage = i18n.getMessage('googleLocationTestWeatherFail', [weatherError.message]);
            showGoogleApiTestResult($result, locationMessage + '<br><span style="color:#c00;">' + warningMessage + '</span>', { color: '#080', background: '#f0fff0' }, true);
        });
}

function testGoogleApiKey(e) {
    e.preventDefault();

    const apiKey = $('#google-api-key').val() || globalSettings.googleApiKey;
    const $result = $('#google-api-key-result');
    if (!apiKey) {
        showGoogleApiTestResult($result, i18n.getMessage('googleLocationTestNoKey'), { color: '#c00', background: '#fff3f3' });
        return;
    }

    showGoogleApiTestResult($result, 'Testing...', { color: '#666', background: '#f5f5f5' });
    fetch('https://www.googleapis.com/geolocation/v1/geolocate?key=' + apiKey, {
        ...
        .then(function (data) {
            if (!data.location) {
                ...
                showGoogleApiTestResult($result, i18n.getMessage('googleLocationTestError', [errorMessage]), { color: '#c00', background: '#fff3f3' });
                return;
            }
            ...
            showGoogleApiTestResult($result, locationMessage, { color: '#080', background: '#f0fff0' }, true);
            appendGoogleWeatherResult(apiKey, data, locationMessage, $result);
        })
        .catch(function (error) {
            showGoogleApiTestResult($result, i18n.getMessage('googleLocationTestError', [error.message]), { color: '#c00', background: '#fff3f3' });
        });
}
