...
const groundstation = (function () {
  ...
  privateScope.updateGui = function () {
    ...
    if (telemetry.gpsFix && telemetry.gpsFix > 1) {
      ...
      if (!privateScope.mapInitiated) {
        ...
        privateScope.cursorStyle = new ol.style.Style({
          image: new ol.style.Icon({
            ...
            src: path.join(
              __dirname,
              './../images/icons/icon_mission_airplane.png'
            )
          })
        });
        privateScope.cursorPosition = new ol.geom.Point(
      ...
      privateScope.cursorStyle
        .getImage()
        .setRotation((telemetry.heading / 360.0) * 6.28318);
    ...
    privateScope.$viewport
      .find('#gs-telemetry-altitude')
      .html((telemetry.altitude / 100.0).toFixed(2) + 'm');
    privateScope.$viewport
      .find('#gs-telemetry-voltage')
      .html((telemetry.voltage / 1000.0).toFixed(2) + 'V');
    privateScope.$viewport.find('#gs-telemetry-sats').html(telemetry.gpsSats);
    privateScope.$viewport
      .find('#gs-telemetry-speed')
      .html(telemetry.groundSpeed.toFixed(0) + 'm/s');
