...
let WaypointCollection = function () {
  ...
  self.update = function (
    ...
    data.forEach(function (element) {
      if (element.isUsed()) {
        ...
        if (
          [MWNP.WPTYPE.JUMP, MWNP.WPTYPE.SET_HEAD, MWNP.WPTYPE.RTH].includes(
            element.getAction()
          )
        ) {
        ...
        if (updateEndFlag) {
          if (element.getNumber() == self.get().length - 1) {
            ...
            element.getNumber() == self.get().length - 2 &&
            element.getEndMission() == 0xa5
          ) {
  ...
  self.getAttachedFromWaypoint = function (waypoint) {
    ...
    data.forEach(function (element) {
      if (
        element.isAttached() &&
        element.getAttachedId() == waypoint.getNumber()
      ) {
        tmpData.push(element);
      }
    });
  ...
  self.addAttachedFromWaypoint = function (waypoint) {
    ...
    let tempWp = new Waypoint(
      waypoint.getNumber() + tmpNumber + 1,
      MWNP.WPTYPE.JUMP,
      0,
      0
    );
  ...
  self.dropAttachedFromWaypoint = function (waypoint, waypointAttachedNumber) {
    data.forEach(function (element) {
      if (
        element.isAttached() &&
        element.getAttachedId() == waypoint.getNumber() &&
        element.getAttachedNumber() == waypointAttachedNumber
      ) {
  ...
  self.isJumpTargetAttached = function (waypoint) {
    ...
    data.forEach(function (element) {
      if (
        element.getAction() == MWNP.WPTYPE.JUMP &&
        element.getP1() == waypoint.getNumber()
      ) {
        lJumptTargetAttached.push(element.getNumber());
      }
    });
    return (
      lJumptTargetAttached.length != 0 && lJumptTargetAttached != 'undefined'
    );
  };
    ...
    while (startCount) {
      ...
      if (
        [
          ...
          MWNP.WPTYPE.LAND
        ].includes(self.getWaypoint(nStart).getAction())
      ) {
        if (reverse) {
          point2measure.push([
            self.getWaypoint(nStart).getLatMap(),
            self.getWaypoint(nStart).getLonMap()
          ]);
        } else {
          point2measure.push(
            ol.proj.fromLonLat([
        ...
        if (
          !Object.keys(jumpDict).includes(
            String(self.getWaypoint(nStart).getNumber())
          )
        ) {
          jumpDict[self.getWaypoint(nStart).getNumber()] = {
        ...
        if (
          ...
          if (
            jumpDict[self.getWaypoint(nStart).getNumber()]['n'] >=
              jumpDict[self.getWaypoint(nStart).getNumber()]['nLoop'] ||
            jumpDict[self.getWaypoint(nStart).getNumber()]['nLoop'] == 0
          ) {
            jumpDict[self.getWaypoint(nStart).getNumber()]['n'] = 0;
            nStart++;
          } else {
            jumpDict[self.getWaypoint(nStart).getNumber()]['n'] =
              jumpDict[self.getWaypoint(nStart).getNumber()]['n'] + 1;
            let nStartTemp =
              jumpDict[self.getWaypoint(nStart).getNumber()]['nStart'];
            nStart = nStartTemp;
          }
        }
      } else {
    ...
    if (nLoop == -1 && display) {
      ...
      point2measure.forEach(function (coord) {
        if (oldCoord != 'undefined' && oldCoord != []) {
          lengthLine.push(
            ol.Sphere.getLength(new ol.geom.LineString([oldCoord, coord]))
          );
        }
        oldCoord = coord;
      });
  ...
  self.getElevation = async function (globalSettings) {
    const [
      ...
      namePoint2measure,
      refPoint2measure
    ] = self.getPoint2Measure(true);
    let lengthMission = self.getDistance(true);
    let totalMissionDistance =
      lengthMission.length >= 1
    ...
    if (point2measure.length <= 2) {
      ...
      Math.trunc(totalMissionDistance / sampleDistance) <= sampleMaxNum &&
      point2measure.length > 2
    ) {
    ...
    let elevation = 'N/A';
    if (globalSettings.mapProviderType == 'bing') {
      let elevationEarthModel = $('#elevationEarthModel').prop('checked')
        ? 'ellipsoid'
        : 'sealevel';

      if (point2measure.length > 1) {
        const response = await fetch(
          'http://dev.virtualearth.net/REST/v1/Elevation/Polyline?points=' +
            point2measure +
            '&heights=' +
            elevationEarthModel +
            '&samples=' +
            String(samples + 1) +
            '&key=' +
            globalSettings.mapApiKey
        );
        const myJson = await response.json();
        elevation = myJson.resourceSets[0].resources[0].elevations;
      } else {
        const response = await fetch(
          'http://dev.virtualearth.net/REST/v1/Elevation/List?points=' +
            point2measure +
            '&heights=' +
            elevationEarthModel +
            '&key=' +
