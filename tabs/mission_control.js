...
var dictOfLabelParameterPoint = {
  1: {
    parameter1: 'Speed (cm/s)',
    parameter2: '',
    parameter3: 'Sea level Ref'
  },
  2: { parameter1: '', parameter2: '', parameter3: '' },
  3: {
    parameter1: 'Wait time (s)',
    parameter2: 'Speed (cm/s)',
    parameter3: 'Sea level Ref'
  },
  4: { parameter1: 'Force land (non zero)', parameter2: '', parameter3: '' },
  5: { parameter1: '', parameter2: '', parameter3: '' },
  6: {
    parameter1: 'Target WP number',
    parameter2: 'Number of repeat (-1: infinite)',
    parameter3: ''
  },
...
TABS.mission_control.initialize = function (callback) {
  ...
  if (CONFIGURATOR.connectionValid) {
    ...
    loadChainer.setChain([
      ...
      function (callback) {
        mspHelper
          .getSetting('nav_fw_land_approach_length')
          .then((data) => {
      ...
      function (callback) {
        mspHelper
          .getSetting('safehome_max_distance')
          .then((data) => {
      ...
      function (callback) {
        mspHelper
          .getSetting('nav_fw_loiter_radius')
          .then((data) => {
    ...
    function update_gpsTrack() {
      ...
      if (FC.GPS_DATA.fix >= 2) {
        if (!cursorInitialized) {
          ...
          breadCrumbLS = new ol.geom.LineString([
        ...
        curPosStyle
          .getImage()
          .setRotation((FC.SENSOR_DATA.kinematics[2] / 360.0) * 6.28318);
  ...
  function loadSettings() {
    ...
    if (missionPlannerSettings) {
      if (
        !missionPlannerSettings.fwApproachLength &&
        settings.fwApproachLength
      ) {
  ...
  function updateSafehomeInfo() {
    let freeSamehomes =
      FC.SAFEHOMES.getMaxSafehomeCount() - FC.SAFEHOMES.safehomeCount();
    $('#availableSafehomes').text(
      freeSamehomes + '/' + FC.SAFEHOMES.getMaxSafehomeCount()
    );
  }
  ...
  function renderSafehomesOnMap() {
    ...
    FC.SAFEHOMES.get().forEach((safehome) => {
      addFwApproach(
        safehome.getLonMap(),
        safehome.getLatMap(),
        FC.FW_APPROACH.get()[safehome.getNumber()],
        safehomeMarkers
      );
    });
    FC.SAFEHOMES.get().forEach((safehome) => {
  ...
  function paintApproachLine(pos1, pos2, color, layers) {
    var line = new ol.geom.LineString([
      ol.proj.fromLonLat([pos1.lon, pos1.lat]),
      ol.proj.fromLonLat([pos2.lon, pos2.lat])
    ]);
  ...
  function paintApproach(
    ...
    var pos2 = calculate_new_cooridatnes(
      pos1,
      direction,
      Math.max(settings.fwLoiterRadius * 4, settings.fwApproachLength / 2)
    );
  ...
  function addFwApproach(lon, lat, fwApproach, layers) {
    if (fwApproach.getLandHeading1() != 0) {
      ...
      paintApproach(
        ...
        settings.fwApproachLength,
        bearing,
        fwApproach.getApproachDirection(),
        layers
      );
    }
    ...
    if (fwApproach.getLandHeading1() > 0) {
      let direction =
        fwApproach.getApproachDirection() == ApproachDirection.LEFT
          ? ApproachDirection.RIGHT
          : ApproachDirection.LEFT;
      paintApproach(
        { lat: lat, lon: lon },
        settings.fwApproachLength,
        fwApproach.getLandHeading1(),
        direction,
        layers
      );
    }
  ...
  function addSafehomeCircles(safehome) {
    ...
    var vectorLayer = new ol.layer.Vector({
      ...
      style: function (iconFeature) {
        ...
        if (safehome.isUsed()) {
          circleStyle.setGeometry(
            new ol.geom.Circle(
              iconFeature.getGeometry().getCoordinates(),
              getProjectedRadius(settings.maxDistSH)
            )
          );
          circleSafeStyle.setGeometry(
            new ol.geom.Circle(
              iconFeature.getGeometry().getCoordinates(),
              getProjectedRadius(Number(settings.safeRadiusSH))
            )
          );
  ...
  function getGeozoneIcon(geozone, number) {
    return new ol.style.Style({
      image: new ol.style.Icon({
        ...
        scale: 0.5,
        src:
          geozone.getType() == GeozoneType.EXCULSIVE
    ...
    var vectorLayer = new ol.layer.Vector({
      ...
      style: function (iconFeature) {
        return [
          getGeozoneIcon(
            zone,
            zone.getShape() == GeozoneShapes.POLYGON
              ? vertex.getNumber()
              : zone.getNumber()
          )
        ];
        ...
        if (zone.getShape() == GeozoneShapes.CIRCULAR) {
          var circleFeature = new ol.Feature({
            geometry: new ol.geom.Circle(
              ol.proj.fromLonLat([
                zone.getFirstVertex().getLonMap(),
                zone.getFirstVertex().getLatMap()
              ]),
              getProjectedRadius(zone.getRadius() / 100)
            ),
            name: 'geozoneCircle'
          });
          ...
          var vectorLayer = new ol.layer.Vector({
            ...
            style: [
              new ol.style.Style({
                stroke: new ol.style.Stroke({
                  color:
                    zone.getType() == GeozoneType.EXCULSIVE
                      ? '#E62121'
                      : '#1DBE0A',
                  width: 3
                })
          ...
          for (let i = 0; i < verticesCount; i++) {
            ...
            paintGeozoneLine(
              pos1,
              pos2,
              zone.getType() == GeozoneType.EXCULSIVE ? '#E62121' : '#1DBE0A',
              prev.getNumber(),
              zone.getNumber()
            );
            prev = current;
  ...
  function geozoneWarning() {
    ...
    for (var i = 0; i < FC.GEOZONES.geozoneCount(); i++) {
      ...
      if (reasons.length > 0) {
        $('#geozoneInvalidContent').append(
          `<div style="display: inline-block">${i18n.getMessage('geozone')} ${zone.getNumber() + 1}: ${reasons.join(', ')}</div><br/>`
        );
  ...
  function updateGeozoneInfo() {
    $('#availableGeozones').text(
      FC.GEOZONES.getMaxZones() -
        FC.GEOZONES.geozoneCount() +
        '/' +
        FC.GEOZONES.getMaxZones()
    );
    $('#availableVertices').text(
      FC.GEOZONES.getMaxVertices() -
        FC.GEOZONES.getUsedVerticesCount() +
        '/' +
        FC.GEOZONES.getMaxVertices()
    );
  }
  ...
  function addGeozone() {
    ...
    FC.GEOZONES.put(
      new Geozone(
        GeozoneType.INCLUSIVE,
        GeozoneShapes.CIRCULAR,
        0,
        10000,
        false,
        20000,
        GeozoneFenceAction.NONE,
        [new GeozoneVertex(0, midLat, midLon)]
      )
    );
  ...
  function updateMultimissionState() {
    ...
    if (!mission.isEmpty() || multimissionCount) {
      if (
        (!multimissionCount || (multimissionCount && !mission.isEmpty())) &&
        multimissionCount < maxMultimissionCount
      ) {
      ...
      if (multimissionCount) {
        ...
        if (singleMissionActive()) {
          totalmultimissionWPs =
            multimission.get().length + mission.get().length;
        ...
        $('#multimissionInfo').text(
          multimissionCount +
            ' missions (' +
            totalmultimissionWPs +
            '/' +
            mission.getMaxWaypoints() +
            ' WPs)'
        );
        document.getElementById('multimissionInfo').style.color =
          totalmultimissionWPs > mission.getMaxWaypoints()
            ? '#FF0000'
            : '#303030';
      } else {
  ...
  function editMultimission() {
    ...
    mission.reinit();
    var tempMissionData = multimission
      .get()
      .slice(startWPCount, endWPCount + 1); // copy selected single mission from MM
    let i = 0;
    tempMissionData.forEach(function (element) {
      // write mission copy to active map mission
      mission.put(element);
  ...
  function getWaypointIcon(waypoint, isEdit) {
    ...
    return new ol.style.Style({
      image: new ol.style.Icon({
        ...
        scale: 0.5,
        src:
          './images/icons/cf_icon_position' +
          (dictofPointIcon[waypoint.getAction()] != ''
            ? '_' + dictofPointIcon[waypoint.getAction()]
            : '') +
          (isEdit ? '_edit' : '') +
          '.png'
      }),
    ...
    mission.get().forEach(function (element) {
      if (!element.isAttached()) {
        ...
        if (element.getAction() == MWNP.WPTYPE.JUMP) {
          ...
          let coord = ol.proj.fromLonLat([
            mission.getWaypoint(jumpWPIndex).getLonMap(),
            mission.getWaypoint(jumpWPIndex).getLatMap()
          ]);
          paintLine(
            oldPos,
            coord,
            ...
            5,
            'Repeat x' +
              (element.getP2() == -1 ? ' infinite' : String(element.getP2())),
            false,
            true
          );
        }
        // If classic WPs is defined with a heading = -1, change Boolean for POI to false. If it is defined with a value different from -1, activate Heading boolean
          ...
          if (element.getP1() == -1) {
            ...
            typeof element.getP1() != 'undefined' &&
            element.getP1() != -1
          ) {
            activatePoi = false;
            activateHead = true;
      ...
      if (element.getAction() == MWNP.WPTYPE.LAND) {
        addFwApproach(
          ...
          element.getLatMap(),
          FC.FW_APPROACH.get()[
            FC.SAFEHOMES.getMaxSafehomeCount() + element.getMultiMissionIdx()
          ],
          lines
        );
      }
    ...
    if (disableMarkerEdit) {
      ...
      if (lengthMission.length >= 1) {
        $('#missionDistance').text(
          lengthMission[lengthMission.length - 1].toFixed(1)
        );
      } else {
  ...
  function renderSafeHomeOptions() {
    if (selectedSafehome && selectedFwApproachSh) {
      ...
      if (
        selectedFwApproachSh.getLandHeading1() == 0 &&
        selectedFwApproachSh.getLandHeading1() == 0 &&
        selectedFwApproachSh.getApproachAltAsl() == 0 &&
        selectedFwApproachSh.getLandAltAsl() == 0
      ) {
      ...
      if (selectedFwApproachSh.getElevation() == 0) {
        (async () => {
          const elevation =
            (await selectedFwApproachSh.getElevationFromServer(
              selectedSafehome.getLonMap(),
              selectedSafehome.getLatMap(),
              globalSettings
            )) * 100;
          selectedFwApproachSh.setElevation(elevation);
          $('#safehomeElevation').text(
            selectedFwApproachSh.getElevation() / 100 + ' m'
          );
        })();
      }

      const $safehomeBox = $safehomeContentBox.find(
        '.missionPlannerSafehomeBox:last-child'
      );
      $safehomeBox
        .find('.spacer_box_title')
        .text(
          i18n.getMessage('safehomeEdit') +
            ' ' +
            (selectedSafehome.getNumber() + 1)
        );
      ...
      changeSwitchery(
        $('#safehomeSeaLevelRef'),
        selectedFwApproachSh.getIsSeaLevelRef()
      );
      $('#safehomeApproachAlt').val(selectedFwApproachSh.getApproachAltAsl());
      $('#safehomeLandAlt').val(selectedFwApproachSh.getLandAltAsl());
      $('#geozoneApproachDirection').val(
        selectedFwApproachSh.getApproachDirection()
      );
      $('#safehomeLandHeading1').val(
        Math.abs(selectedFwApproachSh.getLandHeading1())
      );
      changeSwitchery(
        $('#safehomeLandHeading1Excl'),
        selectedFwApproachSh.getLandHeading1() < 0
      );
      $('#safehomeLandHeading2').val(
        Math.abs(selectedFwApproachSh.getLandHeading2())
      );
      changeSwitchery(
        $('#safehomeLandHeading2Excl'),
        selectedFwApproachSh.getLandHeading2() < 0
      );
      $('#safehomeLandAltM').text(
        selectedFwApproachSh.getLandAltAsl() / 100 + ' m'
      );
      $('#safehomeApproachAltM').text(
        selectedFwApproachSh.getApproachAltAsl() / 100 + ' m'
      );
  ...
  function renderGeozoneOptions() {
    if (selectedGeozone) {
      ...
      const $geozonContent = $geozoneContent.find(
        '.missionPlannerGeozone:last-child'
      );
      $geozonContent
        .find('.spacer_box_title')
        .text(
          i18n.getMessage('missionGeozoneEdit', selectedGeozone.getNumber() + 1)
        );
      ...
      changeSwitchery(
        $('#geozoneSeaLevelRef'),
        selectedGeozone.getSealevelRef()
      );
  ...
  function renderWaypointOptionsTable(waypoint) {
    ...
    mission.getAttachedFromWaypoint(waypoint).forEach(function (element) {
      ...
      for (var i = 1; i <= 3; i++) {
        if (
          dictOfLabelParameterPoint[element.getAction()][
            'parameter' + String(i)
          ] != ''
        ) {
          $row.find('.waypointOptions-p' + String(i)).prop('disabled', false);
          $row
            .find('.waypointOptions-p' + String(i))
            .prop(
              'title',
              dictOfLabelParameterPoint[element.getAction()][
                'parameter' + String(i)
      ...
      GUI.fillSelect(
        $row.find('.waypointOptions-action'),
        waypointOptions,
        waypointOptions.indexOf(MWNP.WPTYPE.REV[element.getAction()])
      );

      $row
        .find('.waypointOptions-action')
        .val(waypointOptions.indexOf(MWNP.WPTYPE.REV[element.getAction()]))
        .on('change', function () {
      ...
      $row
        .find('.waypointOptions-number')
        .text(element.getAttachedNumber() + 1);

      $row
        .find('.waypointOptions-p1')
        .val(
          MWNP.WPTYPE.REV[element.getAction()] == 'JUMP'
            ? mission.convertWaypointToJumpNumber(element.getP1()) + 1
            : element.getP1()
        )
        .on('change', function () {
          if (MWNP.WPTYPE.REV[element.getAction()] == 'SET_HEAD') {
            ...
            if (
              $(this).val() > mission.getNonAttachedList().length ||
              $(this).val() < 1
            ) {
              $(this).val(1);
              GUI.alert(i18n.getMessage('MissionPlannerJumpSettingsCheck'));
            } else if (
              mission.getPoiList().length != 0 &&
              mission.getPoiList()
            ) {
              if (
                mission
                  .getPoiList()
                  .includes(
                    mission.convertJumpNumberToWaypoint(
                      Number($(this).val()) - 1
                    )
                  )
              ) {
          ...
          element.setP1(
            MWNP.WPTYPE.REV[element.getAction()] == 'JUMP'
              ? mission.convertJumpNumberToWaypoint(Number($(this).val()) - 1)
              : Number($(this).val())
          );
          mission.updateWaypoint(element);
          cleanLines();
      ...
      $row
        .find("[data-role='waypointOptions-delete']")
        .attr('data-index', element.getAttachedNumber() + 1);
    });
    GUI.switchery();
    i18n.localize();
    return waypoint;
  ...
  function setView(zoom) {
    var coord = ol.proj.fromLonLat([
      mission.getWaypoint(0).getLonMap(),
      mission.getWaypoint(0).getLatMap()
    ]);
  ...
  function initMap() {
    ...
    app.PlannerSafehomeControl = function (opt_options) {
      ...
      button.innerHTML = ' ';
      button.style =
        "background: url('./images/icons/cf_icon_safehome_white.svg') no-repeat 1px -1px;background-color: rgba(0,60,136,.5);";
    ...
    app.GeozonesControl = function (opt_options) {
      ...
      button.innerHTML = ' ';
      button.style =
        "background: url('./images/icons/cf_icon_geozone_white.svg') no-repeat 1px -1px;background-color: rgba(0,60,136,.5);";
    ...
    app.PlannerElevationControl = function (opt_options) {
      ...
      button.innerHTML = ' ';
      button.style =
        "background: url('./images/icons/cf_icon_elevation_white.svg') no-repeat 1px -1px;background-color: rgba(0,60,136,.5);";
      ...
      var element = document.createElement('div');
      element.className =
        'mission-control-elevation ol-unselectable ol-control';
      element.appendChild(button);
    ...
    app.PlannerMultiMissionControl = function (opt_options) {
      ...
      button.innerHTML = ' ';
      button.style =
        "background: url('./images/icons/cf_icon_multimission_white.svg') no-repeat 1px -1px;background-color: rgba(0,60,136,.5);";
      ...
      var element = document.createElement('div');
      element.className =
        'mission-control-multimission ol-unselectable ol-control';
      element.appendChild(button);
    ...
    app.Drag.prototype.handleDownEvent = function (evt) {
      ...
      tempMarker = map.forEachFeatureAtPixel(
        evt.pixel,
        function (feature, layer) {
          return layer;
        }
    ...
    app.Drag.prototype.handleDragEvent = function (evt) {
      if (
        tempMarker.kind == 'safehomecircle' ||
        tempMarker.kind == 'geozonecircle'
      ) {
        return;
      }
      ...
      if (
        tempMarker.kind == 'waypoint' ||
        tempMarker.kind == 'safehome' ||
        tempMarker.kind == 'home' ||
        tempMarker.kind == 'geozone'
      ) {
      ...
      if (tempMarker.kind == 'waypoint') {
        ...
        if (
          selectedMarker != null &&
          tempMarker.number == selectedMarker.getLayerNumber()
        ) {
        ...
        let tmpVertex = FC.GEOZONES.at(tempMarker.layerNumber).getVertex(
          tempMarker.number
        );
        ...
        let tableBody = $(
          $geozoneContent
            .find('.missionPlannerGeozone')
            .get(tempMarker.layerNumber)
        ).find('#geozoneVerticesTableBody');
        tableBody
          .find(
            'tr:nth-child(' +
              String(tempMarker.number + 1) +
              ') > td > .vertexLon'
          )
          .val(Math.round(coord[0] * 1e7) / 1e7);
        tableBody
          .find(
            'tr:nth-child(' +
              String(tempMarker.number + 1) +
          ...
          .val(Math.round(coord[1] * 1e7) / 1e7);
    ...
    app.Drag.prototype.handleMoveEvent = function (evt) {
      if (this.cursor_) {
        ...
        if (
          feature &&
          feature.name != 'circleFeature' &&
          feature.name != 'circleSafeFeature'
        ) {
    ...
    app.Drag.prototype.handleUpEvent = function (evt) {
      if (tempMarker.kind == 'waypoint') {
        if (
          ...
          (async () => {
            const elevationAtWP = await mission
              .getWaypoint(tempMarker.number)
              .getElevation(globalSettings);
            $('#elevationValueAtWP').text(elevationAtWP);
            const returnAltitude = checkAltElevSanity(
              false,
              mission.getWaypoint(tempMarker.number).getAlt(),
              elevationAtWP,
              mission.getWaypoint(tempMarker.number).getP3()
            );
            mission.getWaypoint(tempMarker.number).setAlt(returnAltitude);

            if (
              mission.getWaypoint(tempMarker.number).getAction() ==
              MWNP.WPTYPE.LAND
            ) {
              let approach =
                FC.FW_APPROACH.get()[
                  FC.SAFEHOMES.getMaxSafehomeCount() +
                    mission.getWaypoint(tempMarker.number).getMultiMissionIdx()
                ];
              if (approach.getIsSeaLevelRef()) {
                if (approach.getElevation() != 0) {
                  approach.setApproachAltAsl(
                    approach.getApproachAltAsl() -
                      approach.getElevation() +
                      elevationAtWP * 100
                  );
                  approach.setLandAltAsl(
                    approach.getLandAltAsl() -
                      approach.getElevation() +
                      elevationAtWP * 100
                  );
                }
                ...
                $('#wpApproachAltM').text(
                  approach.getApproachAltAsl() / 100 + ' m'
                );
              }
            }
        ...
        (async () => {
          ...
          const elevation =
            (await approach.getElevationFromServer(
              safehome.getLonMap(),
              safehome.getLatMap(),
              globalSettings
            )) * 100;
          ...
          if (approach.getIsSeaLevelRef()) {
            if (approach.getElevation() != 0) {
              ...
              approach.setLandAltAsl(
                approach.getLandAltAsl() - approach.getElevation() + elevation
              );
            }
    ...
    if (
      ...
      missionPlannerLastValues.zoom &&
      missionPlannerLastValues.center
    ) {
      map
        .getView()
        .setCenter(ol.proj.fromLonLat(missionPlannerLastValues.center));
    ...
    map.on('click', function (evt) {
      ...
      if (selectedFeature && tempMarker.kind == 'waypoint') {
        ...
        selectedFwApproachWp =
          FC.FW_APPROACH.get()[
            FC.SAFEHOMES.getMaxSafehomeCount() +
              selectedMarker.getMultiMissionIdx()
          ];

        if (
          selectedFwApproachWp.getLandHeading1() == 0 &&
          selectedFwApproachWp.getLandHeading1() == 0 &&
          selectedFwApproachWp.getApproachAltAsl() == 0 &&
          selectedFwApproachWp.getLandAltAsl() == 0
        ) {
        ...
        changeSwitchery(
          $('#pointP3Alt'),
          TABS.mission_control.isBitSet(P3Value, MWNP.P3.ALT_TYPE)
        );
        changeSwitchery(
          $('#pointP3UserAction1'),
          TABS.mission_control.isBitSet(P3Value, MWNP.P3.USER_ACTION_1)
        );
        changeSwitchery(
          $('#pointP3UserAction2'),
          TABS.mission_control.isBitSet(P3Value, MWNP.P3.USER_ACTION_2)
        );
        changeSwitchery(
          $('#pointP3UserAction3'),
          TABS.mission_control.isBitSet(P3Value, MWNP.P3.USER_ACTION_3)
        );
        changeSwitchery(
          $('#pointP3UserAction4'),
          TABS.mission_control.isBitSet(P3Value, MWNP.P3.USER_ACTION_4)
        );

        var altitudeMeters = app.ConvertCentimetersToMeters(
          selectedMarker.getAlt()
        );
        ...
        if (
          tempSelectedMarkerIndex == null ||
          tempSelectedMarkerIndex != selectedMarker.getLayerNumber()
        ) {
          (async () => {
            const elevationAtWP =
              await selectedMarker.getElevation(globalSettings);
            $('#elevationValueAtWP').text(elevationAtWP);
            const returnAltitude = checkAltElevSanity(
              false,
              selectedMarker.getAlt(),
            ...
            selectedFwApproachWp.setIsSeaLevelRef(
              TABS.mission_control.isBitSet(P3Value, MWNP.P3.ALT_TYPE) ? 1 : 0
            );
            $('#wpApproachAlt').val(selectedFwApproachWp.getApproachAltAsl());
            $('#wpLandAlt').val(selectedFwApproachWp.getLandAltAsl);
            $('#wpLandAltM').text(
              selectedFwApproachWp.getLandAltAsl() / 100 + ' m'
            );
            $('#wpApproachAltM').text(
              selectedFwApproachWp.getApproachAltAsl() / 100 + ' m'
            );
        ...
        $('#wpApproachDirection').val(
          selectedFwApproachWp.getApproachDirection()
        );
        $('#wpLandHeading1').val(
        ...
        $('#wpLandHeading2').val(
          Math.abs(selectedFwApproachWp.getLandHeading2())
        );
        changeSwitchery(
          $('#wpLandHeading2Excl'),
          selectedFwApproachWp.getLandHeading2() < 0
        );
        ...
        for (var j in dictOfLabelParameterPoint[selectedMarker.getAction()]) {
          if (dictOfLabelParameterPoint[selectedMarker.getAction()][j] != '') {
            ...
            $('label[for=pointP' + String(j).slice(-1) + ']').html(
              dictOfLabelParameterPoint[selectedMarker.getAction()][j]
            );
          } else {
            $('#pointP' + String(j).slice(-1) + 'class').fadeOut(300);
          }
        }
        selectedMarker = renderWaypointOptionsTable(selectedMarker);
        $('#EditPointNumber').text(
          'Edit point ' + String(selectedMarker.getLayerNumber() + 1)
        );
        ...
        selectedFeature &&
        tempMarker.kind == 'line' &&
        tempMarker.selection &&
        !disableMarkerEdit
      ) {
        ...
        let tempWp = new Waypoint(
          ...
          Math.round(tempWpCoord[1] * 10000000),
          Math.round(tempWpCoord[0] * 10000000),
          Number(settings.alt),
        ...
        if (homeMarkers.length && HOME.getAlt() != 'N/A') {
          (async () => {
            ...
            tempWp.setAlt(
              checkAltElevSanity(false, settings.alt, elevationAtWP, false)
            );
        ...
        selectedFeature &&
        tempMarker.kind == 'geozoneline' &&
        tempMarker.selection
      ) {
        if (
          FC.GEOZONES.getUsedVerticesCount() + 1 >
          FC.GEOZONES.getMaxVertices()
        ) {
          GUI.alert(i18n.getMessage('missionGeozoneMaxVerticesReached'));
          return;
        ...
        let tmpVertex = new GeozoneVertex(
          tempMarker.number + 1,
          Math.round(tempCoord[1] * 1e7),
          Math.round(tempCoord[0] * 1e7)
        );
        FC.GEOZONES.at(tempMarker.layerNumber).insertVertex(
          tempMarker.number + 1,
          tmpVertex
        );
        selectedGeozone = FC.GEOZONES.at(tempMarker.layerNumber);
        ...
        if (mission.get().length == 0) {
          tempWp.setMultiMissionIdx(
            multimissionCount == 0 ? 0 : multimissionCount - 1
          );
          FC.FW_APPROACH.clean(
            FC.SAFEHOMES.getMaxSafehomeCount() + tempWp.getMultiMissionIdx()
          );
        } else {
          tempWp.setMultiMissionIdx(
            mission.getWaypoint(mission.get().length - 1).getMultiMissionIdx()
          );
        }
    ...
    setInterval(function () {
      let width = $('#missionMap canvas').width(),
        height = $('#missionMap canvas').height();
    ...
    $('#showHideActionButton').on('click', function () {
      var src =
        $(this).children().attr('class') === 'ic_hide' ? 'ic_show' : 'ic_hide';
      ...
      if (selectedMarker) {
        ...
        if (
          [
            ...
            MWNP.WPTYPE.LAND
          ].includes(selectedMarker.getAction())
        ) {
    ...
    $('#pointLat').on('change', function (event) {
      if (selectedMarker) {
        selectedMarker.setLat(
          Math.round(Number($('#pointLat').val()) * 10000000)
        );
        mission.updateWaypoint(selectedMarker);
        mission.update(singleMissionActive());
        refreshLayers();
        selectedFeature = markers[selectedMarker.getLayerNumber()]
          .getSource()
          .getFeatures()[0];
        selectedFeature.setStyle(getWaypointIcon(selectedMarker, true));
        plotElevation();
    ...
    $('#pointLon').on('change', function (event) {
      if (selectedMarker) {
        selectedMarker.setLon(
          Math.round(Number($('#pointLon').val()) * 10000000)
        );
        mission.updateWaypoint(selectedMarker);
        mission.update(singleMissionActive());
        refreshLayers();
    ...
    $('#pointP3Alt').on('change', function (event) {
      if (selectedMarker) {
        ...
        P3Value = TABS.mission_control.setBit(
          P3Value,
          MWNP.P3.ALT_TYPE,
          $('#pointP3Alt').prop('checked')
        );
        (async () => {
          const elevationAtWP =
            await selectedMarker.getElevation(globalSettings);
          ...
          if (P3Value != selectedMarker.getP3()) {
            ...
            let groundClearance =
              100 * Number($('#groundClearanceValueAtWP').text());
            ...
            if ($('#pointP3Alt').prop('checked')) {
              ...
              selectedMarker.setAlt(
                groundClearance + 100 * (elevationAtWP - elevationAtHome)
              );
            }

            if (
              selectedMarker.getAction() == MWNP.WPTYPE.LAND &&
              selectedFwApproachWp &&
              selectedFwApproachWp.getIsSeaLevelRef() !=
                $('#pointP3Alt').prop('checked')
            ) {
              let oldElevation = 0;
              if (selectedFwApproachWp.getIsSeaLevelRef()) {
              ...
              if ($('#pointP3Alt').prop('checked')) {
                selectedFwApproachWp.setApproachAltAsl(
                  selectedFwApproachWp.getApproachAltAsl() -
                    oldElevation +
                    elevationAtWP * 100
                );
                selectedFwApproachWp.setLandAltAsl(
                  selectedFwApproachWp.getLandAltAsl() -
                    oldElevation +
                    elevationAtWP * 100
                );
              } else {
                selectedFwApproachWp.setApproachAltAsl(
                  selectedFwApproachWp.getApproachAltAsl() - elevationAtWP * 100
                );
                selectedFwApproachWp.setLandAltAsl(
                  selectedFwApproachWp.getLandAltAsl() - elevationAtWP * 100
                );
              }
              selectedFwApproachWp.setElevation(elevationAtWP * 100);
              selectedFwApproachWp.setIsSeaLevelRef(
                $('#pointP3Alt').prop('checked') ? 1 : 0
              );
              $('#wpApproachAlt').val(selectedFwApproachWp.getApproachAltAsl());
              $('#wpLandAlt').val(selectedFwApproachWp.getLandAltAsl());
            }
          ...
          selectedMarker.setAlt(returnAltitude);
          $('#pointAlt').val(selectedMarker.getAlt());
          let altitudeMeters = app.ConvertCentimetersToMeters(
            selectedMarker.getAlt()
          );
          $('#altitudeInMeters').text(` ${altitudeMeters}m`);
    ...
    $('#pointP3UserAction1').on('change', function (event) {
      if (selectedMarker) {
        if (disableMarkerEdit) {
          changeSwitchery(
            $('#pointP3UserAction1'),
            TABS.mission_control.isBitSet(
              selectedMarker.getP3(),
              MWNP.P3.USER_ACTION_1
            )
          );
        }

        var P3Value = TABS.mission_control.setBit(
          selectedMarker.getP3(),
          MWNP.P3.USER_ACTION_1,
          $('#pointP3UserAction1').prop('checked')
        );
        selectedMarker.setP3(P3Value);
    ...
    $('#pointP3UserAction2').on('change', function (event) {
      if (selectedMarker) {
        ...
        var P3Value = TABS.mission_control.setBit(
          selectedMarker.getP3(),
          MWNP.P3.USER_ACTION_2,
          $('#pointP3UserAction2').prop('checked')
        );
        selectedMarker.setP3(P3Value);
    ...
    $('#pointP3UserAction3').on('change', function (event) {
      if (selectedMarker) {
        ...
        var P3Value = TABS.mission_control.setBit(
          selectedMarker.getP3(),
          MWNP.P3.USER_ACTION_3,
          $('#pointP3UserAction3').prop('checked')
        );
        selectedMarker.setP3(P3Value);
    ...
    $('#pointP3UserAction4').on('change', function (event) {
      if (selectedMarker) {
        ...
        var P3Value = TABS.mission_control.setBit(
          selectedMarker.getP3(),
          MWNP.P3.USER_ACTION_4,
          $('#pointP3UserAction4').prop('checked')
        );
        selectedMarker.setP3(P3Value);
    ...
    $('#wpApproachAlt').on('change', (event) => {
      if (selectedMarker && selectedFwApproachWp) {
        ...
        if (
          checkApproachAltitude(
            altitude,
            $('#pointP3Alt').prop('checked'),
            Number($('#elevationValueAtWP').text())
          )
        ) {
          selectedFwApproachWp.setApproachAltAsl(
    ...
    $('#wpLandHeading1Excl').on('change', (event) => {
      if (selectedMarker && selectedFwApproachWp) {
        ...
        if ($('#wpLandHeading1Excl').prop('checked')) {
          selectedFwApproachWp.setLandHeading1(
            -Math.abs(selectedFwApproachWp.getLandHeading1())
          );
        } else {
          selectedFwApproachWp.setLandHeading1(
    ...
    $('#wpLandHeading2Excl').on('change', (event) => {
      if (selectedMarker && selectedFwApproachWp) {
        ...
        if ($('#wpLandHeading2Excl').prop('checked')) {
          selectedFwApproachWp.setLandHeading2(
            -Math.abs(selectedFwApproachWp.getLandHeading2())
          );
        } else {
          selectedFwApproachWp.setLandHeading2(
    ...
    /////////////////////////////////////////////
    $waypointOptionsTableBody.on(
      'click',
      "[data-role='waypointOptions-delete']",
      function (event) {
        if (selectedMarker) {
          mission.dropAttachedFromWaypoint(
            selectedMarker,
            $(event.currentTarget).attr('data-index') - 1
          );
    ...
    $('#addSafehome').on('click', () => {
      ...
      let midLat = Math.round(ol.proj.toLonLat(mapCenter)[1] * 1e7);
      FC.SAFEHOMES.put(
        new Safehome(FC.SAFEHOMES.safehomeCount(), 1, midLat, midLon)
      );
    ...
    $('#safehomeLatitude').on('change', (event) => {
      if (selectedSafehome && selectedFwApproachSh) {
        selectedSafehome.setLat(
          Math.round(Number($(event.currentTarget).val()) * 1e7)
        );
        renderSafeHomeOptions();
        cleanSafehomeLayers();
        renderSafehomesOnMap();
    ...
    $('#safehomeSeaLevelRef').on('change', (event) => {
      ...
      if (
        selectedSafehome &&
        selectedFwApproachSh &&
        isChecked != selectedFwApproachSh.getIsSeaLevelRef()
      ) {
        ...
        (async () => {
          ...
          if (isChecked) {
            selectedFwApproachSh.setApproachAltAsl(
              selectedFwApproachSh.getApproachAltAsl() + elevation
            );
            selectedFwApproachSh.setLandAltAsl(
              selectedFwApproachSh.getLandAltAsl() + elevation
            );
          } else {
            selectedFwApproachSh.setApproachAltAsl(
              selectedFwApproachSh.getApproachAltAsl() - elevation
          ...
          $('#safehomeApproachAlt').val(
            selectedFwApproachSh.getApproachAltAsl()
          );
          $('#safehomeLandAlt').val(selectedFwApproachSh.getLandAltAsl());
          $('#safehomeLandAltM').text(
            selectedFwApproachSh.getLandAltAsl() / 100 + ' m'
    ...
    $('#safehomeApproachAlt').on('change', (event) => {
      if (selectedFwApproachSh) {
        ...
        if (
          checkApproachAltitude(
            altitude,
            $('#safehomeSeaLevelRef').prop('checked'),
            Number($('#safehomeElevation').text())
          )
        ) {
          selectedFwApproachSh.setApproachAltAsl(
            Number($(event.currentTarget).val())
    ...
    $('#safehomeLandHeading1Excl').on('change', (event) => {
      if (selectedFwApproachSh && !lockShExclHeading) {
        selectedFwApproachSh.setLandHeading1(
          selectedFwApproachSh.getLandHeading1() * -1
        );
        cleanSafehomeLayers();
        renderSafehomesOnMap();
      }
    ...
    $('#safehomeLandHeading2Excl').on('change', (event) => {
      if (selectedFwApproachSh && !lockShExclHeading) {
        selectedFwApproachSh.setLandHeading2(
          selectedFwApproachSh.getLandHeading2() * -1
        );
        cleanSafehomeLayers();
        renderSafehomesOnMap();
      }
    ...
    $('#geozoneShape').on('change', (event) => {
      if (selectedGeozone) {
        if ($(event.currentTarget).val() == GeozoneShapes.CIRCULAR) {
          ...
          if (
            FC.GEOZONES.getUsedVerticesCount() + 2 >
            FC.GEOZONES.getMaxVertices()
          ) {
            GUI.alert(i18n.getMessage('missionGeozoneMaxVerticesReached'));
            renderGeozoneOptions();
    ...
    $('#geozoneSeaLevelRef').on('change', (event) => {
      ...
      if (selectedGeozone && isChecked != selectedGeozone.getSealevelRef()) {
        ...
        (async () => {
          ...
          const elevation = await selectedGeozone.getElevationFromServer(
            vertex.getLonMap(),
            vertex.getLatMap(),
            globalSettings
          );
          ...
          if (isChecked) {
            selectedGeozone.setMinAltitude(
              Number(selectedGeozone.getMinAltitude()) + elevation * 100
            );
            selectedGeozone.setMaxAltitude(
              Number(selectedGeozone.getMaxAltitude()) + elevation * 100
            );
          } else {
            selectedGeozone.setMinAltitude(
              Number(selectedGeozone.getMinAltitude()) - elevation * 100
    ...
    $('#elevationEarthModel').on('change', function (event) {
      if (globalSettings.mapProviderType == 'bing') {
        ...
        settings.bingDemModel = $('#elevationEarthModel').prop('checked')
          ? true
          : false;
        saveSettings();
      }
    ...
    $('#removeAllPoints').on('click', function () {
      if (
        markers.length &&
        GUI.confirm(i18n.getMessage('confirm_delete_all_points'))
      ) {
        ...
        for (
          let i = FC.SAFEHOMES.getMaxSafehomeCount();
          i < FC.FW_APPROACH.getMaxFwApproachCount();
          i++
        ) {
    ...
    $('#removePoint').on('click', function () {
      if (selectedMarker) {
        if (mission.isJumpTargetAttached(selectedMarker)) {
          ...
          mission.getAttachedFromWaypoint(selectedMarker) &&
          mission.getAttachedFromWaypoint(selectedMarker).length != 0
        ) {
          if (
            GUI.confirm(i18n.getMessage('confirm_delete_point_with_options'))
          ) {
            mission
              .getAttachedFromWaypoint(selectedMarker)
    ...
    $('#loadMissionButton').on('click', function () {
      let message = multimissionCount
        ? 'confirm_overwrite_multimission_file_load_option'
        : 'confirm_delete_all_points';
      if (
        (markers.length || multimissionCount) &&
        !GUI.confirm(i18n.getMessage(message))
      )
        return;
      removeAllWaypoints();
      $(this).addClass('disabled');
    ...
    $('#saveSettings').on('click', function () {
      ...
      if (
        settings.safeRadiusSH != oldSafeRadiusSH &&
        $('#showHideSafehomeButton').is(':visible')
      ) {
        cleanSafehomeLayers();
        renderSafehomesOnMap();
        $('#SafeHomeSafeDistance').text(settings.safeRadiusSH);
  ...
  function loadMissionFile(filename) {
    ...
    fs.readFile(filename, (err, data) => {
      ...
      xml2js
        .Parser({ explicitChildren: true, preserveChildrenOrder: true })
        .parseString(data, (err, result) => {
              ...
              if (nodemission.$$ && nodemission.$$.length) {
                for (var i = 0; i < nodemission.$$.length; i++) {
                  ...
                  if (node['#name'].match(/version/i) && node.$) {
                    ...
                    node['#name'].match(/meta/i) ||
                    (node['#name'].match(/mwp/i) && node.$)
                  ) {
                    for (var attr in node.$) {
                      if (attr.match(/mission/i)) {
                        ...
                        mission.setCenterLon(
                          parseFloat(node.$[attr]) * 10000000
                        );
                      } else if (attr.match(/cy/i)) {
                        mission.setCenterLat(
                          parseFloat(node.$[attr]) * 10000000
                        );
                      } else if (attr.match(/home\-x/i)) {
                        HOME.setLon(
                          Math.round(parseFloat(node.$[attr]) * 10000000)
                        );
                    ...
                    for (var attr in node.$) {
                      if (attr.match(/no/i)) {
                        ...
                        if (node.$[attr].match(/WAYPOINT/i)) {
                          ...
                          node.$[attr].match(/PH_UNLIM/i) ||
                          node.$[attr].match(/POSHOLD_UNLIM/i)
                        ) {
                          point.setAction(MWNP.WPTYPE.POSHOLD_UNLIM);
                        } else if (
                          node.$[attr].match(/PH_TIME/i) ||
                          node.$[attr].match(/POSHOLD_TIME/i)
                        ) {
                    ...
                    for (var attr in node.$) {
                      if (attr.match(/index/i)) {
                        ...
                        fwApproach.setApproachDirection(
                          node.$[attr] == 'left' ? 0 : 1
                        );
                        ...
                        fwApproach.setIsSeaLevelRef(
                          parseBooleans(node.$[attr]) ? 1 : 0
                        );
                      }
                    }
                    FC.FW_APPROACH.insert(
                      fwApproach,
                      FC.SAFEHOMES.getMaxSafehomeCount() + idx
                    );
                  }
                }
              }
          ...
          if (missionEndFlagCount > 1) {
            if (
              multimissionCount &&
              !GUI.confirm(i18n.getMessage('confirm_multimission_file_load'))
            ) {
              mission.flush();
              return;
            } else {
          ...
          if (Object.keys(mission.getCenter()).length !== 0) {
            var coord = ol.proj.fromLonLat([
              mission.getCenter().lon / 10000000,
              mission.getCenter().lat / 10000000
            ]);
            map.getView().setCenter(coord);
    ...
    mission.get().forEach(function (waypoint) {
      ...
      var point = {
        $: {
          ...
          parameter1:
            MWNP.WPTYPE.REV[waypoint.getAction()] == 'JUMP'
              ? waypoint.getP1() + 1
              : waypoint.getP1(),
      ...
      if (approach.getLandHeading1() != 0 || approach.getLandHeading2() != 0) {
        var item = {
          $: {
            ...
            'approach-direction':
              approach.getApproachDirection() == 0 ? 'left' : 'right',
  ...
  function sendWaypointsToFC(saveEeprom) {
    ...
    function setMissionIndex() {
      let activeIndex = singleMissionActive()
        ? 1
        : $('#activeNissionIndex').text();
      mspHelper.setSetting(
        ...
        function () {
          MSP.send_message(
            ...
            function () {
              GUI.log(
                i18n.getMessage('multimission_active_index_saved_eeprom')
              );
            }
          );
        }
  ...
  function updateTotalInfo() {
    if (CONFIGURATOR.connectionValid) {
      ...
      $('#availablePoints').text(
        availableWPs + '/' + mission.getMaxWaypoints()
      );
      $('#missionValid').html(
        mission.getValidMission()
          ? i18n.getMessage('armingCheckPass')
          : i18n.getMessage('armingCheckFail')
      );
    }
  }
  ...
  function checkAltElevSanity(
    ...
    AbsAltCheck =
      typeof AbsAltCheck == 'boolean'
        ? AbsAltCheck
        : TABS.mission_control.isBitSet(AbsAltCheck, MWNP.P3.ALT_TYPE);
    ...
    if (AbsAltCheck) {
      ...
      if (checkAltitude / 100 + elevationAtHome < elevation) {
        if (resetAltitude) {
          ...
          altitude =
            currentGroundClearance + 100 * (elevation - elevationAtHome);
        }
      }
    ...
    document.getElementById('groundClearanceAtWP').style.color =
      groundClearance < settings.alt / 100 ? '#FF0000' : '#303030';
      ...
      if (mission.isEmpty()) {
        ...
        (async () => {
          const [
            ...
            namePoint2measure,
            refPoint2measure
          ] = await mission.getElevation(globalSettings);
          let x_elevation = Array.from(
            Array(samples + 1),
            (_, i) => (i * totalMissionDistance) / samples
          );
          var trace_WGS84 = {
            x: x_elevation,
            y: elevation,
          ...
          let y_missionElevation = altPoint2measure.map(
            (x, i) => x / 100 + HOME.getAlt() * (1 - refPoint2measure[i])
          );
          let y_elevationReference = refPoint2measure.map((x, i) =>
            x == 1 ? 'WGS84' : 'Take-off Home'
          );
          ...
          var layout = {
            ...
            yaxis: {
              ...
              range: [
                -10 +
                  Math.min(
                    Math.min(...y_missionElevation),
                    Math.min(...elevation)
                  ),
                10 +
                  Math.max(
                    Math.max(...y_missionElevation),
                    Math.max(...elevation)
                  )
              ]
            },
