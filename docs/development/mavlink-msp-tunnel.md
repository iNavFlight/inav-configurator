...
- **Decoder reset.** On every lapse, `resetDecoders()` resets the MSP decoder (`MSP.resetDecoder()`) and the link's reassembly clock. A lost chunk leaves the decoder mid-frame, and the next reply would otherwise be eaten as its payload. `MavlinkLink` also resets the MSP decoder when a chunk arrives more than 1000 ms after the previous one, mirroring the FC's partial-frame timeout.
...
**Uptime rule:** `MSP2_INAV_MISC2` starts with the FC's on-time in seconds (u32, `fc_msp.c`). An uptime shorter than the time since the reboot request means the FC rebooted. The uptime is what decides, because a link fade looks like a reboot and a fast reboot can hide between two heartbeats.
